...
public class GameDriver {
   ...
   * It reads the world details, spaces, items, and pet from the input stream.
   *
  ...
  private static World parseWorld(BufferedReader reader) throws IOException {
    ...
    // Read pet information (name only for now)
    ...
    for (int i = 0; i < numberOfSpaces; i++) {
      ...
      // Create the Space with an id (using 'i' as a unique id for simplicity)
      ImSpace space = new Space(i, spaceName, upperLeftRow, 
    ...
    // Create the World object without a pet for now
    World world = new World(rows, cols, worldName, 
        spaces, items, targetCharacter, null, new ArrayList<>());


    // Create the Pet with the now-existing World reference
    Pet pet = new Pet(petName, targetCharacter, world);
    pet.moveTo(world.getSpace(targetCharacter.getCurrentSpace())); 

    // Now set the pet in the World instance
    world.setPet(pet);
